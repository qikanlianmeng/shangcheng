...
define(function(require, exports, module) {
    ...
    $get('/api2/order/order_detail', { 'id': order_id }, function(data) {
        if (data.status) {
            ...
            $get('/api2/order/get_send_info', { 'order_id': order_id }, function(dataCode) {
                var $logistics = $('.logistics');
                if (dataCode.status == 1) {
                    $logistics.find('p.color-main').text(dataCode.list[0].context);
                    $logistics.find('p.color-999').text(dataCode.list[0].time);
                } else {
