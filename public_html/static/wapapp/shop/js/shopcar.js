...
define(function(require, exports, module) {
    mui('footer.mui-bar-tab').on('tap', 'a', function() {
    ...
    var specData = {};
    //购物车列表数据
    $get('/api2/goods/get_cart_list', {}, function(data) {
            if (data.data) {
                $shopcarKong.hide();
                $shopcaropera.show();
                $shopcarBottom.show();
                ...
                for (var i = 0; i < cart_list.length; i++) {
                    var g = cart_list[i].goods_group;
                    cart_list[i].g_g = g;
                    cart_list[i].goods_group = g == 1 ? '样品区商品' : g == 2 ? '代营区商品' : g == 3 ? '订货区商品' : '';
    ...
    $shopcarBottom.on('click', '.allSelect', function() {
            ...
            if ($this.find('.iconfont').hasClass('icon-chenggong')) {
                $get('/api2/goods/change_cart_selected_all', { 'selected': 0 }, function(data) {
                    if (data.status) {
                ...
                $get('/api2/goods/change_cart_selected_all', { 'selected': 1 }, function(data) {
                    if (data.status) {
    ...
    $shopcarList.on('click', '.iconfont', function() {
        ...
        $get('/api2/goods/change_cart_selected', { 'id': id }, function(data) {
            if (data.status) {
    ...
    $('#shopcarList').on('click', '.addNum', function() {
        ...
        if ($this.hasClass('color-999')) {
            ...
            $get('/api2/goods/change_cart_num', { 'id': goodsid }, function(data) {
                if (data.status) {
    ...
    $('#shopcarList').on('click', '.subNum', function() {
        ...
        if ($this.hasClass('color-999')) {
            ...
            $get('/api2/goods/change_cart_num', { 'id': goodsid, 'mode': '-' }, function(data) {
                if (data.status) {
                    var _num = $this.siblings('.num').text();
                    _num--;
                    $this.parents('.priceNum').find('.orinum span').text(_num);
    ...
    function getgoodsNum() {
        ...
        if (_num === car_num) {
            $('.allSelect').find('i').removeClass('icon-yuan').addClass('icon-chenggong color-main');
        }
    }
    // 计算购物车商品的数量
    ...
    $('.shopcarList').on('click', '.del', function() {
        ...
        var ind = common.confirm('你确定要删除该商品吗？', function() {
            $('.layermbox').remove();
            $get('/api2/goods/remove_cart', { 'id': goodsid }, function(data) {
                if (data.status) {
                    $parentLi.remove();
    ...
    $('.shopcarList').on('click', '.atten', function() {
        ...
        group = $this.parents('li').attr('goodsgroup');
        $get('/api2/goods/collection', { 'id': id, group: group }, function(data) {
            common.toast(data.msg);
            if (data.status) {
                if ($this.text() == '收藏') {
    ...
    $('.account').on('click', '.goCal', function() {
        ...
        $get('/api2/order/confirm_order', {}, function(data) {
            if (data.status == 1) {
                window.location.href = '/wapapp/order/firm_order';
            } else {
