...
class Ad extends Base
{
   ...
   //幻灯片分类
	...
	public function slidecat_add(){
	 return $this->fetch();
	}
	public function slidecat_edit(){
	 return $this->fetch();
	}
	public function slidecat_del(){
	...
	public function my_test(){
		//$res = Db::query('select uid,sum(sum_money-money) as duoyu from think_log_income group by uid');
		//echo json_encode($res);
		/* set_time_limit(0);
		$str ='';
		$arr = json_decode($str,true);
		foreach($arr as $v){
			$log_income = ['uid'=>$v['uid'],'money'=>-$v['duoyu'],'type'=>12,'create_time'=>time(),'act_admin'=>1,'act_msg'=>'扣除重复收益'];
			Db::name('log_income')->insert($log_income);
			Db::name('member')->where('id',$v['uid'])->setDec('money',$v['duoyu']);
		}
		echo 'done'; */
	}
