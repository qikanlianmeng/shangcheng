...
class Order extends Base
{
     ...
     * 提交订单
     */
    public function add_order($address_id, $pay_points = 0, $user_money = 0, $content = '')
    {
        if (!$user_id = get_uid()) {
            return json(['status' => 0, 'msg' => '未登录，请先登录']);
        }
        ...
        $address = $address['data'];
        //计算商品价格
        $goodsPrice = $priceModel->calculate_price($user_id, $cartList, 0, $address['province'], $address['city'], $address['district'], $pay_points, $user_money);
        if ($goodsPrice['status'] == 0) {
            return json(['status' => 0, 'msg' => $goodsPrice['msg']]);
        }
    ...
    public function order_list($type = 0, $page = 1, $num = 8)
    {
        ...
        $orderList = Db::name('order')->where($map)->order('order_id desc')->page($page, $num)->select();
        if ($orderList) {
            //查询订单商品信息
     ...
     * 支付订单
     * @param $order_id 订单ID
     * @param $pay_type 支付方式  alipay  支付宝   wxpay  微信
     * @return array
     */
    public function pay_order($order_id, $pay_type)
    {
        if (!$user_id = get_uid()) {
            return json(['status' => 0, 'msg' => '未登录，请先登录']);
        }
    ...
    public function get_send_info($order_id) {
        if (!$user_id = get_uid()) {
            return json(['status' => -1, 'msg' => '未登录，请先登录']);
        }

        if (!$code = Db::name('order')->where(['order_id' => $order_id, 'user_id' => $user_id])->value('invoice_no')) {
            return json(['status' => -1, 'msg' => '暂无匹配订单或该订单未发货']);
        }
        $host = "http://jisukdcx.market.alicloudapi.com";
        $path = "/express/query";
        $method = "GET";
        $appcode = "ab6beb51418241cc914f129fd8e9d74a";
        $headers = array();
