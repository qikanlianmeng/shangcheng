...
class PayNotifyAct
{
    ...
    public function chongzhi($order_number, $param)
    {
        ...
        if (!empty($info) && $info->status == 0) {
            ...
            Db::name('log_income')->insert(['uid'=>$info->uid,'money'=>$info->money,'create_time'=>time(),'type'=>9]);
            if ($tag['code'] == 0) {
                Log::error($tag['msg']);
                return false;
            } else {
     ...
     * @return bool
     */
    public function cash($order_number, $param)
    {

        if(!Db::name('log_cash')->where('id',$order_number)->update(['status'=>3,'update_time'=>time()])) {
            $msg = '修改提现申请状态出错';
            Log::error($msg);
            return $msg;
        }
